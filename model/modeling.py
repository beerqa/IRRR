  ...
  def __init__(self,
	...
	# it is important that the embedding codes remain fp32.
        ...
        if prev_query_ids is not None:            
          (self.prev_embedding_output, self.prev_embedding_table) = embedding_lookup(
            input_ids=prev_query_ids,
            vocab_size=2,
            embedding_size=config.hidden_size,
            initializer_range=config.initializer_range,
            word_embedding_name="prev_query_embeddings",
            use_one_hot_embeddings=use_one_hot_embeddings,
            zero_first_vector=True)
          self.embedding_output = self.embedding_output + self.prev_embedding_output

        if sentence_ids != None:
          (self.sentence_embedding_output, _) = embedding_lookup(
              input_ids=sentence_ids,
              vocab_size=30,
              embedding_size=config.hidden_size,
              initializer_range=config.initializer_range,
              word_embedding_name="sentence_embeddings",
              use_one_hot_embeddings=use_one_hot_embeddings)
          self.embedding_output = self.embedding_output + self.sentence_embedding_output              
              
        # Add positional embeddings and token type embeddings, then layer
        # normalize and perform dropout.
        self.embedding_output, self.token_type_table, self.position_table = embedding_postprocessor(
...
def embedding_lookup(input_ids,
                     ...
                     initializer_range=0.02,
                     word_embedding_name="word_embeddings",
                     use_one_hot_embeddings=False, embedding_table=None, zero_first_vector=False):
  ...
  if embedding_table is None:
    if zero_first_vector:
      zero_vector = tf.zeros([1, embedding_size], dtype=tf.float32)
      embedding_table = tf.get_variable(
        name=word_embedding_name,
        shape=[vocab_size-1, embedding_size],
        initializer=create_initializer(initializer_range))
      embedding_table = tf.concat([zero_vector, embedding_table], axis=0)
    else:
      embedding_table = tf.get_variable(
        name=word_embedding_name,
...
def embedding_postprocessor(input_tensor,
  ...
  if use_position_embeddings:
    ...
    # perform a slice.
    if predict_sequence_i is -1: 
      position_embeddings = tf.slice(full_position_embeddings, [0, 0],
                                    [seq_length, width])
    else:
      position_embeddings = full_position_embeddings[predict_sequence_i:predict_sequence_i + 1]
  ...
  return output, token_type_table, full_position_embeddings
