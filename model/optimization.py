...
def create_optimizer(loss, init_lr, num_train_steps, num_warmup_steps, hvd=None, manual_fp16=False, use_fp16=False, num_accumulation_steps=1,
  ...
  if num_accumulation_steps > 1:
      ...
      new_global_step = tf.cond(tf.math.logical_and(update_step, tf.cast(hvd.allreduce(tf.cast(batch_finite, tf.int32)) if hvd is not None else tf.cast(batch_finite, tf.int32), tf.bool)), lambda: global_step+1, lambda: global_step)
